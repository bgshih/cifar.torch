...
local function ResBlock(nIn, nOut, subsample, dropRate)
  local concat
  if subsample then
    local conv = nn.Sequential()
    conv:add(ConvBnRelu(nIn, nOut, 3, 2, 1))
    conv:add(nn.Dropout(dropRate))
...
local function AddResGroup(nIn, nOut, n, firstSub, dropRate)
  resnet:add(ResBlock(nIn, nOut, firstSub, dropRate))
  for i = 1, n-1 do
    resnet:add(ResBlock(nOut, nOut, false, dropRate))
  end
end

local n = 18
resnet:add(ConvBnRelu(3,16,3,1,1)):add(nn.Dropout(0.3))
AddResGroup(16,16,n,false,0.4)
AddResGroup(16,32,n,true,0.4)
AddResGroup(32,64,n,true,0.5)
